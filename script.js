...
function scribdStep(url) {
    if(url.includes('document') || url.includes('doc')) {
        const blurredPages = Array.from(document.querySelectorAll('.outer_page.blurred_page')),
...
function studocuStep(url) {
    if(url.includes('document')) {
        ...
        for(let child of documentWrapperNodes) {
            if(child.nodeName === 'DIV' && child.firstChild?.id !== 'page-container-wrapper') {
                child.remove();
            }
        }
        ...
        documentContainer.innerHTML = documentContainer.innerHTML.replace(/style="user-select:none;filter:blur\(4px\)"/gm, '');
